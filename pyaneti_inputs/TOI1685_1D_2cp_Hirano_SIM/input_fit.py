...
fit_t0 = ['g']*nplanets
fit_P  = ['g']*nplanets
fit_ew1= ['f']*nplanets   #We fix sqrt(e) sin w, it works only if is_ew = True
fit_ew2= ['f']*nplanets   #We fix sqrt(e) cos w, it works only if is_ew = True
fit_e  = ['f']*nplanets   #We fix e, it works only if is_ew = False
fit_w  = ['f']*nplanets   #We fix w, it works only if is_ew = False
fit_b  = ['u']*nplanets   #We fix the impact factor
fit_a  = ['u']*nplanets   #We fit a with gaussian priors (given by the stellar parameters)
...
#Planet b values from Matthew's updated transit fit (TESS + Muscat) 
#and the 2nd planet is from Hirano 2021
min_t0  = [9910.93830,8827.46]
max_t0  = [0.00038,0.90]
min_P   = [0.66913923,2.5891]
max_P   = [0.00000040,0.0069]
...
fit_krv = [None]*5 #Define the list with 7 elements
fit_krv[0] = 'u' #A_0
fit_krv[1] = 'f' #A_1
